...
async def debug_db_connection():
    ...
    try:
        ...
        async with engine.begin() as conn:
            ...
            """))
            workspace_tables = result.fetchall()
        ...
        return {
            ...
            "tables_in_current_schema": [{"name": t[0], "schema": t[1]} for t in tables],
            "workspace_tables_found": [{"name": t[0], "schema": t[1]} for t in workspace_tables]
        }
...
async def debug_database():
    ...
    try:
        ...
        async with engine.begin() as conn:
            ...
            # Try to query workspace table
            try:
                result = await conn.execute(text("SELECT COUNT(*) FROM workspace"))
                count = result.fetchone()
                workspace_count = count[0] if count else "Error"
            except Exception as e:
                workspace_count = f"Error: {str(e)}"
        ...
        return {
            ...
            "workspace_table_exists": len(tables) > 0,
            "workspace_tables": [{"name": t[0], "schema": t[1]} for t in tables],
